...
core.exportProblem = function(problem, opts) {
  ...
  for (let i = 0; i < problem.templateMeta.params.length; i++) {
    ...
    switch (param.type) {
      case 'integer[]':
        codeArray.push(`        List<?> ${param.name}List = (List<?>)invocable.invokeFunction("parse", args[${i}]);`);
        ...
        break;
      case 'integer':
        codeArray.push(`        int ${param.name} = (int)invocable.invokeFunction("parse", args[${i}]);`);
        paramArray.push(param.name);
        break;
      }
  }
...
${codeArray.join('\n')}
        ...
        var res = s.${problem.templateMeta.name}(
          ${paramArray.join(', ')}
        );
