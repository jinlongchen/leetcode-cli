...
`;
      ...
      case 'java': {
        ...
        for (let i = 0; i < problem.templateMeta.params.length; i++) {
          ...
          switch (param.type) {
            ...
            case 'list<integer>':
              ...
              break;
            case 'long':
              codeArray.push(`        long ${param.name} = (long)invocable.invokeFunction("parse", testcase[${i}]);`);
              paramArray.push(param.name);
              break;
            case 'character':
              codeArray.push(`        char ${param.name} = (char)invocable.invokeFunction("parse", testcase[${i}]);`);
              paramArray.push(param.name);
              break;
            case 'double':
              codeArray.push(`        double ${param.name} = (double)invocable.invokeFunction("parse", testcase[${i}]);`);
              paramArray.push(param.name);
              break;
            case 'boolean':
              codeArray.push(`        boolean ${param.name} = (boolean)invocable.invokeFunction("parse", testcase[${i}]);`);
              paramArray.push(param.name);
              break;
            case 'integer':
        ...
        `);
            ...
            case 'ListNode[]':
              extClassName = 'ListNode'
              codeArray.push(`        List<?> ${param.name}ListListTmp = (List<?>)invocable.invokeFunction("parse", testcase[${i}]);`);
            ...
            case 'TreeNode':
              ...
              codeArray.push(`        TreeNode ${param.name} = TreeNode.construct(testcase[${i}]);`);
              paramArray.push(param.name);
              break;
            case 'list<TreeNode>':
              extClassName = 'TreeNode';
              codeArray.push(`        List<TreeNode> ${param.name} = new ArrayList<>();`);
              codeArray.push(`        List<?> treesList = (List<?>)invocable.invokeFunction("parse", testcase[${i}]);`);
        ...
        System.out.println();`;
          ...
          case 'void':
            resEquation = '';
            // codeArray.push(`        System.out.println();`);
            break;
          default:
