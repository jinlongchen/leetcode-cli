...
`;
      ...
      case 'java': {
        ...
        `);
              ...
              break;
            case 'ListNode[]':
              codeArray.push(`        List<?> ${param.name}ListTmp = (List<?>)invocable.invokeFunction("parse", testcase[${i}]);`);
              codeArray.push(`        ListNode[] ${param.name}Array = new ListNode[${param.name}ListTmp.size()];`);
              codeArray.push(`        for (int i = 0; i < ${param.name}ListTmp.size(); i++) {`);
              codeArray.push(`          Object list = ${param.name}ListTmp.get(i);`);
            ...
            case 'character[][]':
              codeArray.push(`        List<?> ${param.name}ListTmp = (List<?>)invocable.invokeFunction("parse", testcase[${i}]);`);
              codeArray.push(`        char[][] ${param.name}Array = new char[${param.name}ListTmp.size()][];`);
              codeArray.push(`        for (int i = 0; i < ${param.name}ListTmp.size(); i++) {`);
              codeArray.push(`          Object list = ${param.name}ListTmp.get(i);`);
