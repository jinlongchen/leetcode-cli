...
function clearCacheIfTchanged(needTranslation) {
  ...
  let problemDate = cache.get(h.KEYS.problemDate);

  const translationConfig = cache.get(h.KEYS.translation);

  let today = getDate();

  if (!translationConfig || translationConfig['useEndpointTranslation'] != needTranslation || today != problemDate) {
    ...
    cache.set(h.KEYS.problemDate, today);
