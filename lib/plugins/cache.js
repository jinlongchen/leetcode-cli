...
function clearCacheIfTchanged(needTranslation) {
  let verKey = h.KEYS.version + require('../../package.json').version;
  let version = cache.get(verKey);
  const translationConfig = cache.get(h.KEYS.translation);

  if (!translationConfig || translationConfig['useEndpointTranslation'] != needTranslation || !version) {
...
plugin.getProblems = function (needTranslation, cb) {
  ...
  plugin.next.getProblems(needTranslation, function(e, problems) {
    ...
    // because leetcode's data is updated very frequently, so let's disable cache.
